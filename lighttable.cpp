...
LightTable::LightTable(QWidget *parent) : QTableWidget(parent)
{
    ...
    connect(&hue, SIGNAL(dataUpdated()), this, SLOT(update()));
}
...
void LightTable::doubleClick(const QModelIndex index)
{
    if (index.column() == 2)
    {
        hue.setOn(this->item(this->currentRow(),7)->text().toInt(),
    ...
    else if (index.column() == 4)
    {
        ...
        if (ok)
            hue.setBri(this->item(this->currentRow(),7)->text().toInt(), bri*2.54 );
    }
    else if (index.column() == 5)
    {
        hue.setColorLoop(this->item(this->currentRow(),7)->text().toInt(),
